...
function generateCustomTheme(hue: number): { light: Record<string, string>; dark: Record<string, string> } {
  return {
    light: {
      "--primary": `${hue} 50% 35%`,
      "--accent": `${(hue + 15) % 360} 45% 50%`,
      "--ring": `${hue} 50% 35%`,
      "--reading": `${(hue + 10) % 360} 50% 42%`,
      "--finished": `${hue} 50% 35%`,
      "--want-to-read": `${(hue + 20) % 360} 40% 55%`,
    },
    dark: {
      "--primary": `${hue} 45% 50%`,
      "--accent": `${(hue + 15) % 360} 45% 50%`,
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const applyCurrentTheme = useCallback((id: string, hue: number | null, isDark: boolean) => {
    if (id === "custom" && hue !== null) {
      ...
      applyThemeVars(isDark ? t.dark : t.light);
    } else {
      const preset = PRESETS.find(p => p.id === id);
      if (preset) {
        applyThemeVars(isDark ? preset.dark : preset.light);
      } else {
        clearThemeVars();
      }
    }
