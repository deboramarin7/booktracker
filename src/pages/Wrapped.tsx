      ...
      {[1, 2, 3, 4, 5].map(i => (
        <Star
          ...
          className={`h-6 w-6 transition-all duration-500 ${
            i <= Math.round(rating) ? "text-amber-400 fill-amber-400 scale-110" : "text-white/20"
          }`}
...
export default function Wrapped() {
  ...
  const bestMonth = useMemo(() => {
    ...
    return entries[0] ? { month: MONTH_NAMES[Number(entries[0][0])], count: Number(entries[0][1]) } : null;
  }, [yearBooks]);
