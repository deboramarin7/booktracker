        ...
        <TabsContent value="habits" className="space-y-8 mt-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div
                key={stat.label}
                className="text-center p-4 rounded-xl bg-muted/30 border border-border/20"
              >
        ...
        <TabsContent value="calendar" className="space-y-6 mt-6">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="text-center p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
              <p className="text-3xl font-bold text-emerald-600 dark:text-emerald-400 font-display">
                {booksStarted.size}
              </p>
          ...
          <div className="rounded-xl border border-border/30 bg-card overflow-hidden">
            <div className="grid grid-cols-7">
              {DAY_NAMES.map((name) => (
                <div
                  key={name}
                  className="p-2 text-center text-xs font-medium text-muted-foreground border-b bg-muted/20"
                >
                  {name}
                </div>
              ))}
            ...
            <div className="grid grid-cols-7">
              {Array.from({ length: firstDay }).map((_, i) => (
                <div
                  key={`empty-${i}`}
                  className="min-h-[80px] border-b border-r border-border/20 bg-muted/5"
                />
              ))}
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span
                            className={cn(
                              "text-xs font-medium",
                              isTodayCal(day) ? "text-primary font-bold" : "text-foreground"
                            )}
                          >
                            {day}
                          </span>
                          ...
                          {data.reading.length > 0 && (
                            <div className="flex flex-wrap gap-1 mb-0.5">
                              ...
                              {data.reading.length > 3 && (
                                <span className="text-[8px] text-muted-foreground self-end">
                                  +{data.reading.length - 3}
                                </span>
                              )}
                            </div>
                      ...
                      {data.reading.length === 0 ? (
                        ...
                        <div className="space-y-1">
                          ...
                          {data.reading
                            .filter(
                              (b) =>
                                !data.started.some((s) => s.title === b.title) &&
                                !data.finished.some((f) => f.title === b.title)
                            )
                            .map((b, i) => (
