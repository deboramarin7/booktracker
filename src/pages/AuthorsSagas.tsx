...
import { BookOpen, LibraryBig, Search, Sparkles, UsersRound, X } from "lucide-react";
...
function CollectionCard({ entry, kind, onOpen }: { entry: CollectionEntry; kind: "author" | "saga"; onOpen: () => void }) {
  const label = kind === "author" ? "libros leídos" : "títulos leídos";
  return <article onClick={onOpen} className="group cursor-pointer rounded-2xl border border-border/45 bg-card p-4 transition-all duration-200 hover:-translate-y-1 hover:border-primary/45 hover:shadow-[0_12px_28px_rgba(0,0,0,0.16)]">
...
export default function AuthorsSagas() {
  ...
  const [selected, setSelected] = useState<{ entry: CollectionEntry; kind: "author" | "saga" } | null>(null);
  ...
  return <div className="space-y-8 pb-8">
    ...
    <div className="grid gap-8 xl:grid-cols-2">
      <section><div className="mb-4 flex items-end justify-between"><div><p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-primary"><UsersRound className="h-4 w-4" /> Autores</p><h3 className="mt-1 font-display text-2xl font-semibold">Las voces que te acompañan</h3></div><span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">{visibleAuthors.length}</span></div><div className="space-y-3">{visibleAuthors.length ? visibleAuthors.map((entry) => <CollectionCard key={entry.name} entry={entry} kind="author" onOpen={() => setSelected({ entry, kind: "author" })} />) : <EmptyState kind="author" />}</div></section>
      <section><div className="mb-4 flex items-end justify-between"><div><p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-amber-100"><LibraryBig className="h-4 w-4" /> Sagas</p><h3 className="mt-1 font-display text-2xl font-semibold">Universos en orden</h3></div><span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">{visibleSagas.length}</span></div><div className="space-y-3">{visibleSagas.length ? visibleSagas.map((entry) => <CollectionCard key={entry.name} entry={entry} kind="saga" onOpen={() => setSelected({ entry, kind: "saga" })} />) : <EmptyState kind="saga" />}</div></section>
    </div>
    {selected && <div className="fixed inset-0 z-[100] grid place-items-center p-4"><button onClick={() => setSelected(null)} className="absolute inset-0 bg-black/70 backdrop-blur-sm" aria-label="Cerrar" /><section className="relative max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-3xl border border-border bg-card p-6 shadow-2xl"><button onClick={() => setSelected(null)} className="absolute right-4 top-4 p-2 text-muted-foreground"><X className="h-4 w-4" /></button><p className="text-xs font-semibold uppercase tracking-[.16em] text-primary">{selected.kind === "author" ? "Autora / autor" : "Saga"}</p><h2 className="mt-2 pr-8 font-display text-3xl font-semibold">{selected.entry.name}</h2><p className="mt-2 text-sm text-muted-foreground">{selected.entry.total} títulos terminados</p><div className="mt-6 space-y-2">{selected.entry.titles.map((title, index) => <div key={`${title}-${index}`} className="flex items-center gap-3 rounded-xl border border-border/40 bg-muted/20 px-3 py-2.5"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-primary/10 text-xs font-semibold text-primary">{index + 1}</span><p className="text-sm font-medium">{title}</p></div>)}</div></section></div>}
  </div>;
}
