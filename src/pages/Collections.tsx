...
export default function Collections() {
  ...
  return <div className="space-y-8 pb-8">
    ...
    {!hasCollections ? <div className="rounded-3xl border border-dashed border-primary/30 bg-card/50 px-6 py-16 text-center"><div className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-primary/10 text-3xl">✦</div><h3 className="mt-5 font-display text-2xl font-semibold">Empieza tu primer rincón</h3><p className="mx-auto mt-2 max-w-sm text-sm text-muted-foreground">Navidad, romance, fantasía acogedora… tus libros pueden pertenecer a todas las historias que quieras.</p><button onClick={() => setOpen(true)} className="mt-6 text-sm font-semibold text-primary hover:underline">Crear colección</button></div> : <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{collections.map((collection) => { const covers = booksFor(collection.id); return <article key={collection.id} onClick={() => setSelectedCollection(collection)} className="group relative cursor-pointer overflow-hidden rounded-3xl border border-border/45 bg-card p-5 transition-transform hover:-translate-y-1"><div className="absolute inset-x-0 top-0 h-1" style={{ backgroundColor: collection.color }} /><button onClick={(event) => { event.stopPropagation(); remove(collection.id); }} aria-label={`Eliminar ${collection.name}`} className="absolute right-4 top-4 rounded-lg p-2 text-muted-foreground opacity-0 transition-opacity hover:bg-destructive/10 hover:text-destructive group-hover:opacity-100"><Trash2 className="h-4 w-4" /></button><div className="flex items-start gap-4"><div className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl text-2xl" style={{ backgroundColor: `${collection.color}22`, color: collection.color }}>{collection.emoji}</div><div className="min-w-0"><h3 className="font-display text-xl font-semibold">{collection.name}</h3><p className="mt-1 text-xs text-muted-foreground">{countFor(collection.id)} libro{countFor(collection.id) === 1 ? "" : "s"} · Abrir colección</p></div></div>{collection.description && <p className="mt-4 min-h-10 text-sm text-muted-foreground">{collection.description}</p>}<div className="mt-5 flex h-24 items-end gap-2">{covers.length ? covers.map((book: any) => <img key={book.id} src={book.coverUrl} alt="" className="h-24 w-16 rounded-md object-cover shadow-lg" />) : <div className="flex h-24 w-full items-center justify-center rounded-xl border border-dashed border-border text-xs text-muted-foreground"><BookOpen className="mr-2 h-4 w-4" /> Aún no hay libros</div>}</div></article>; })}</div>}
    {selectedCollection && <div className="fixed inset-0 z-[100] overflow-y-auto bg-background"><div className="mx-auto max-w-6xl p-4 sm:p-8"><button onClick={() => setSelectedCollection(null)} className="mb-7 flex items-center gap-2 rounded-xl border border-border px-3 py-2 text-sm text-muted-foreground hover:text-foreground"><X className="h-4 w-4" /> Volver a colecciones</button><section className="rounded-3xl border border-border/45 bg-card p-6 sm:p-8" style={{ borderTopColor: selectedCollection.color, borderTopWidth: 3 }}><div className="flex items-start gap-4"><div className="grid h-16 w-16 shrink-0 place-items-center rounded-2xl text-3xl" style={{ backgroundColor: `${selectedCollection.color}22`, color: selectedCollection.color }}>{selectedCollection.emoji}</div><div><p className="text-xs font-semibold uppercase tracking-[.16em]" style={{ color: selectedCollection.color }}>Tu colección</p><h2 className="mt-1 font-display text-3xl font-semibold">{selectedCollection.name}</h2><p className="mt-2 text-sm text-muted-foreground">{selectedCollection.description || `${countFor(selectedCollection.id)} historias reunidas aquí.`}</p></div></div></section><div className="mt-8 grid grid-cols-2 gap-5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">{links.filter((link) => link.collection_id === selectedCollection.id).map((link) => books.find((book) => book.id === link.book_id)).filter(Boolean).map((book: any) => <div key={book.id}><img src={book.coverUrl} alt={book.title} className="aspect-[2/3] w-full rounded-xl object-cover shadow-lg" /><p className="mt-2 truncate font-semibold text-sm">{book.title}</p><p className="truncate text-xs text-muted-foreground">{book.author}</p></div>)}</div><details className="mt-10 rounded-2xl border border-border/40 bg-card p-4"><summary className="cursor-pointer text-sm font-semibold text-primary">Gestionar libros de esta colección</summary><div className="mt-4 grid gap-2 sm:grid-cols-2">{books.map((book) => { const selected = links.some((link) => link.collection_id === selectedCollection.id && link.book_id === book.id); return <button key={book.id} type="button" onClick={() => toggleBook(selectedCollection.id, book.id)} className={`flex items-center justify-between rounded-xl px-3 py-2 text-left text-sm ${selected ? "bg-primary/15 text-primary" : "bg-card hover:bg-muted"}`}><span className="truncate">{book.title}</span><span>{selected ? "✓" : "+"}</span></button>; })}</div></details></div></div>}
